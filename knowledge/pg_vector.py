...
class PgVector(VectorDb):
    ...
    def get_all_records(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List[Document]: List of all documents in the table.
        """
        try:
            with self.Session() as sess, sess.begin():
    ...
    def delete_record_by_id(self, record_id: int) -> bool:
      """
      Delete a record from the table based on its ID.
      ...
      try:
          with self.Session() as sess, sess.begin():
              stmt = delete(self.table).where(self.table.c.id == record_id) # type: ignore
          ...
          logger.error(f"Error deleting record with ID {record_id}: {e}")
          return False
    
    def delete_record_by_name(self, record_name: str) -> bool:
      """
      Delete a record from the table based on its name.

      Args:
          record_name (str): The name of the record to delete.
      ...
      try:
          with self.Session() as sess, sess.begin():
              stmt = delete(self.table).where(self.table.c.name == record_name)
              result = sess.execute(stmt)
              if result.rowcount == 0:
                  logger.warning(f"No record found with name: {record_name}")
                  return False
              logger.info(f"Record with name {record_name} deleted successfully.")
              return True
      except Exception as e:
          logger.error(f"Error deleting record with name {record_name}: {e}")
          return False
